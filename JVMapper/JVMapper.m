...
+ (id)populateObject:(id)obj fromDictionary:(NSDictionary *)dict {
    ...
    if ([obj respondsToSelector:@selector(jvExcludedProperties)]) {
        excludeArray = [obj jvExcludedProperties];
    }
    ...
    if ([obj respondsToSelector:@selector(jvDataKeysForClassProperties)]) {
        dataKeysForProperties = [obj jvDataKeysForClassProperties];
    }
    ...
    for (id dataKey in dict) {
        ...
        if (![dataKey isKindOfClass:[NSString class]]) {
        ...
        if (![JVMapper hasBasicPrefix:propertyType] &&
            ...
            if ([obj conformsToProtocol:@protocol(JVMappingCD)]) {
                childObj = [[objCls alloc] initWithDictionary:value context:[obj managedObjectContext]];
            } else if ([obj conformsToProtocol:@protocol(JVMappingSQ)]) {
                childObj = [[objCls alloc] initWithDictionaryAndSave:value];
            } else {
            ...
            if (![value isKindOfClass:[NSArray class]]) {
                ...
                if ([propertyType isEqualToString:@"NSNumber"]) {
                    ...
                    [obj setValue:@((([value isKindOfClass:[NSString class]] && [value isEqualToString:@"true"]) || (BOOL) [value intValue])) forKey:property];
                ...
                } else if ([obj respondsToSelector:@selector(jvItemClassForArrayProperty:)]) {
                    // Get the class of item inside the array
                    Class itemCls = [obj jvItemClassForArrayProperty:property];
...
+ (NSDictionary *)dictionaryForObject:(id)obj exclude:(NSArray *)excludeArray {
    NSMutableDictionary *dict = [JVMapper mutableDictionaryForObject:obj exclude:excludeArray];
    return [NSDictionary dictionaryWithDictionary:dict];
}

+ (NSMutableDictionary *)mutableDictionaryForObject:(id)obj {
    return [JVMapper mutableDictionaryForObject:obj exclude:nil];
}
...
+ (NSMutableDictionary *)mutableDictionaryForObject:(id)obj exclude:(NSArray *)excludeArray {
    ...
    for (NSString *property in properties) {
        ...
        // If "value" is custom class, we will try to parse this custom class to NSDictionary
        NSString *propertyType = properties[property];

        if (![JVMapper hasBasicPrefix:propertyType] && value) {
            value = [JVMapper mutableDictionaryForObject:value exclude:nil];

            [objDict setValue:value forKey:dataKey];
            ...
            if ([propertyType isEqualToString:@"NSNumber"]) {
                ...
                [objDict setValue:myNumber forKey:dataKey];

            } else if ([propertyType isEqualToString:@"c"]) {

                [objDict setValue:@([value charValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"i"]) {

                [objDict setValue:@([value intValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"s"]) {

                [objDict setValue:@([value shortValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"l"]) {

                [objDict setValue:@([value longValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"q"]) {

                [objDict setValue:@([value longLongValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"C"]) {

                [objDict setValue:@([value unsignedCharValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"I"]) {

                [objDict setValue:@([value unsignedIntValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"S"]) {

                [objDict setValue:@([value unsignedShortValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"L"]) {

                [objDict setValue:@([value unsignedLongValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"Q"]) {

                [objDict setValue:@([value unsignedLongLongValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"f"]) {

                [objDict setValue:@([value floatValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"d"]) {

                [objDict setValue:@([value doubleValue]) forKey:dataKey];

            } else if ([propertyType isEqualToString:@"B"]) {

                [objDict setValue:@((([value isKindOfClass:[NSString class]] && [value isEqualToString:@"true"]) || (BOOL) [value intValue])) forKey:dataKey];
